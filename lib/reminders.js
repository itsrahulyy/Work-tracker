...
function parseVideoRows(table) {
  ...
  const indexes = {
    ...
    done: headers.findIndex((header) => header === "VIDEO DONE" || header === "VIDEO STATUS"),
  };
  ...
  for (const row of table.rows) {
    ...
    if (!/[A-Za-z]/.test(String(city).trim())) {
      continue;
    }
...
function parseVideoDoneValue(value) {
  ...
  if (["closed", "complete", "completed", "done", "received video"].includes(normalized)) {
    return { done: 1, total: 1 };
  }

  if (["in progress", "in-progress", "pending", "open", "not started"].includes(normalized)) {
    return { done: 0, total: 1 };
  }
